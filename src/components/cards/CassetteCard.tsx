...
function GlobeIcon() {
  return (
    <svg width="40" height="40" viewBox="0 0 40 40" fill="none" aria-hidden>
      ...
      <ellipse cx="20" cy="20" rx="8" ry="18" stroke={COLORS.peach} strokeWidth="1" />
      <line x1="2" y1="20" x2="38" y2="20" stroke={COLORS.peach} strokeWidth="1" />
      <line x1="20" y1="2" x2="20" y2="38" stroke={COLORS.peach} strokeWidth="1" />
      <ellipse cx="20" cy="12" rx="14" ry="5" stroke={`${COLORS.peach}60`} strokeWidth="0.75" />
      <ellipse cx="20" cy="28" rx="14" ry="5" stroke={`${COLORS.peach}60`} strokeWidth="0.75" />
    </svg>
  );
}
...
// Dot matrix pattern
function DotMatrix({ rows = 8, cols = 4 }: { rows?: number; cols?: number }) {
  return (
    <div className="grid gap-[3px]" style={{ gridTemplateColumns: `repeat(${cols}, 4px)` }}>
      {Array.from({ length: rows * cols }).map((_, i) => (
        <motion.div
          key={i}
          className="w-1 h-1 rounded-full"
          style={{ background: i % 3 === 0 ? COLORS.peach : `${COLORS.white}40` }}
          initial={{ opacity: 0.3 }}
...
function ScanLine() {
  return (
    <motion.div
      ...
      style={{ background: `linear-gradient(90deg, transparent, ${COLORS.peach}60, transparent)` }}
...
export default function CassetteCard({ event, index }: CassetteCardProps) {
  return (
    <motion.div
          ...
          <motion.div whileTap={{ scale: 0.985 }} className="relative h-full">
            ...
            >
              ...
              <div className="relative p-4 pb-3">
                ...
                {/* Main title - Large stacked text */}
                <div className="mb-2">
                  <div
                    className="font-display text-4xl md:text-5xl font-black tracking-tight leading-[0.9] uppercase"
                    style={{ color: COLORS.white }}
                  >
                    {event.name.split(" ").slice(0, 2).map((word, i) => (
                      <div key={i}>{word}</div>
                    ))}
                  </div>
                </div>
                ...
                <div className="flex items-center gap-3 mb-4">
                  ...
                  <div className="flex-1 flex items-center gap-1">
                    <div className="flex-1 h-[1px]" style={{ background: `${COLORS.white}30` }} />
                    <div className="w-2 h-2 rotate-45" style={{ border: `1px solid ${COLORS.peach}` }} />
                    <div className="flex-1 h-[1px]" style={{ background: `${COLORS.white}30` }} />
                  </div>
                </div>
              ...
              <div className="px-4 py-3 flex items-center gap-2 flex-wrap">
                ...
                >
                  <svg width="12" height="12" viewBox="0 0 12 12" fill="none">
                    <path d="M2 6h8M7 3l3 3-3 3" stroke={COLORS.accent} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
                  </svg>
                </motion.div>
                ...
                <div className="flex items-end justify-between gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-2 mt-1">
                      <span
                        className="font-display text-[9px] tracking-[0.25em] uppercase"
                        style={{ color: `${COLORS.white}60` }}
                      >
                        Category:
                      </span>
                  ...
                  <div
                    className="text-[8px] font-display tracking-[0.35em] uppercase"
                    style={{
